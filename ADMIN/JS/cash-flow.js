...
function getSaleStatus(data){
    return String(getValue(data,["status","saleStatus","paymentStatus"],"completed")).toLowerCase();
}
...
function normalizeCashFlow(id,data){
    ...
    const typeValue=String(getValue(data,["type"],cashIn>0?"in":"out")).toLowerCase();
    const type=typeValue==="in"?"in":"out";
    return{
        id,
        date:getDateValue(data)||new Date(0),
        type,
        category:String(getValue(data,["category","type"],type==="in"?"Other Income":"Expense")),
        description:String(getValue(data,["description","remarks","notes"],"")),
        reference:String(getValue(data,["reference","transactionId","transactionNumber"],id)),
        cashIn,
        cashOut
    };
}
...
function normalizeSale(id,data){
    ...
    if(["cancelled","canceled","void","voided","refunded"].includes(status)){
        return null;
    }
    ...
    const customer=String(
        getValue(data,["customerName","customer","customer_name"],"Walk-in Customer")
    );

    const payment=String(
        getValue(data,["paymentMethod","payment","method"],"")
    );
...
function rebuildCashFlow(){
    ...
    const generatedSales=salesData
        ...
        .filter(Boolean)
        .filter(item=>!existingSaleReferences.has(String(item.reference)));
...
function updateSummary(){
    ...
    if(totalCashInElement){
        totalCashInElement.textContent=formatMoney(totalCashIn);
    }

    if(totalCashOutElement){
        totalCashOutElement.textContent=formatMoney(totalCashOut);
    }

    if(netCashFlowElement){
        netCashFlowElement.textContent=formatMoney(net);
    }

    if(cashBalanceElement){
        cashBalanceElement.textContent=formatMoney(net);
    }
    ...
    for(const record of cashFlowData){
        ...
        if(record.category==="Expense"||record.category==="Other Expense"){
    ...
    const otherIncomeCash=document.getElementById("otherIncomeCash");
    const expenseCash=document.getElementById("expenseCash");
    const purchaseCash=document.getElementById("purchaseCash");
    const refundCash=document.getElementById("refundCash");

    if(salesCash){
        salesCash.textContent=formatMoney(sales);
    }

    if(otherIncomeCash){
        otherIncomeCash.textContent=formatMoney(otherIncome);
    }

    if(expenseCash){
        expenseCash.textContent=formatMoney(expenses);
    }

    if(purchaseCash){
        purchaseCash.textContent=formatMoney(purchases);
    }

    if(refundCash){
        refundCash.textContent=formatMoney(refunds);
    }
}
...
function renderTable(){
    ...
    for(const record of pageRecords){
        ...
        const cashIn=record.cashIn>0
            ?`<span class="cash-in">+${formatMoney(record.cashIn)}</span>`
            :"<span>—</span>";

        const cashOut=record.cashOut>0
        ...
        row.innerHTML=`
            <td>
                <div class="flow-id">${escapeHTML(record.id)}</div>
            </td>
            <td>
                <div class="flow-date">${escapeHTML(formatDate(record.date))}</div>
            </td>
            <td>
...
function filterCashFlow(){
    ...
    if(!searchElement||!dateElement||!typeElement||!categoryElement){
        return;
    }

    const search=searchElement.value.trim().toLowerCase();
    const dateFilter=dateElement.value;
    const typeFilter=typeElement.value;
    const categoryFilter=categoryElement.value;

    filteredCashFlow=cashFlowData.filter(record=>{
        const searchable=`${record.id} ${record.description} ${record.category} ${record.reference}`.toLowerCase();

        const matchesSearch=!search||searchable.includes(search);
        const matchesDate=matchesDateFilter(record.date,dateFilter);
    ...
    cashFlowChart=new Chart(canvas,{
        ...
        data:{
            ...
            datasets:[
                {
                    label:"Cash In",
                    data:data.map(item=>item.cashIn),
                    borderColor:"#16803c",
                ...
                {
                    label:"Cash Out",
                    data:data.map(item=>item.cashOut),
                    borderColor:"#d74343",
                    backgroundColor:"rgba(215,67,67,0.06)",
                    fill:true,
                    tension:0.35,
                    borderWidth:2,
                    pointRadius:3
...
async function saveCashOut(event){
    ...
    if(!category||!description||!amount||amount<=0||!dateValue){
    ...
    try{
        ...
        console.error("Firebase cash out save error:",error);
...
async function saveCashIn(event){
    ...
    try{
        ...
        console.log("[StockMaster] Saving Cash In:",cashInData);
        ...
        console.log("[StockMaster] Cash In saved:",docRef.id);
        ...
        console.error("[StockMaster] Firebase Cash In error:",error);
...
function loadFirebaseData(user){
    ...
    unsubscribeCashFlow=onSnapshot(
        collection(db,"cashFlow"),
        snapshot=>{
            cashFlowData=snapshot.docs.map(
                doc=>normalizeCashFlow(doc.id,doc.data())
            );

            console.log(
                "[StockMaster] Cash Flow records loaded:",
                cashFlowData.length
            );
        ...
        error=>{
            console.error(
                "[StockMaster] Cash Flow Firebase error:",
                error
            );
    ...
    unsubscribeSales=onSnapshot(
        ...
        error=>{
            console.error(
                "[StockMaster] Sales Firebase error:",
                error
            );
...
document.getElementById("cashflowSearch")?.addEventListener(
    "input",
    filterCashFlow
);
...
document.getElementById("typeFilter")?.addEventListener(
    "change",
    filterCashFlow
);
...
document.getElementById("nextPage")?.addEventListener(
    ...
    ()=>{
        const totalPages=Math.ceil(
            filteredCashFlow.length/recordsPerPage
        );
...
document.getElementById("addExpenseButton")?.addEventListener(
    "click",
    ()=>{
...
document.getElementById("addCashInButton")?.addEventListener(
    "click",
    ()=>{
        console.log("[StockMaster] Add Cash In clicked");
        setDefaultCashInDate();
        cashInModal?.classList.add("show");
    }
);

document.getElementById("closeExpenseModal")?.addEventListener(
    "click",
    closeExpenseModal
);

document.getElementById("cancelExpense")?.addEventListener(
    "click",
    closeExpenseModal
);

document.getElementById("closeCashInModal")?.addEventListener(
    "click",
    closeCashInModal
);

document.getElementById("cancelCashIn")?.addEventListener(
    "click",
    closeCashInModal
);
...
document.getElementById("cashInForm")?.addEventListener(
    "submit",
    saveCashIn
);

document.getElementById("chartPeriod")?.addEventListener(
    "change",
    updateChart
);
