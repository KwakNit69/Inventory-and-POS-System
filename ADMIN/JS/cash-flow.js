...
function addPaymentAmount(result, account, value) {
    ...
    if (
        normalized &&
        ACCOUNT_NAMES.includes(normalized) &&
        amount > 0
    ) {
...
function getPaymentBreakdown(data) {
    ...
    if (Array.isArray(splitPayments)) {
        splitPayments.forEach(payment => {
            ...
            if (
                normalized &&
                Number(amount) > 0 &&
                amounts[normalized] === 0
            ) {
                amounts[normalized] = Number(amount);
            }
        });
    } else if (
        splitPayments &&
        typeof splitPayments === "object"
    ) {
        Object.entries(splitPayments).forEach(([key, value]) => {
            ...
            const normalized = normalizeAccount(key);
    ...
    const total =
        ACCOUNT_NAMES.reduce(
            (sum, account) =>
                sum + (Number(amounts[account]) || 0),
            0
        );
...
function getPaymentBreakdownTotal(breakdown) {
    return ACCOUNT_NAMES.reduce(
        (sum, account) =>
            sum + (Number(breakdown[account]) || 0),
        0
    );
}
...
function getPrimaryPaymentMethod(data) {
    ...
    const methods = ACCOUNT_NAMES.filter(
        account =>
            Number(breakdown[account]) > 0
    );
...
function normalizeSale(id, data) {
    ...
    return {
        ...
        category: "Sale",
        description:
            `POS Sale${paymentMethod ? ` - ${paymentMethod}` : ""}${customer ? ` - ${customer}` : ""}`,
        reference,
        cashIn,
        cashOut: 0,
        source: "sale",
...
function calculateAccountBalances() {
    ...
    for (const record of cashFlowData) {
        ...
        if (
            ...
            if (
                account &&
                ACCOUNT_NAMES.includes(account)
            ) {
                balances[account] += amount;
            } else {
...
function updateAccountBalances() {
    ...
    window.stockMasterAccountBalances = {
        ...balances,
        totalFunds
    };

    console.log(
        "[StockMaster] Account Balances:",
        window.stockMasterAccountBalances
    );
}
...
function renderTable() {
    ...
    for (const record of pageRecords) {
        ...
        const accountText =
            record.account
                ? `<div class="flow-account">${escapeHTML(record.account)}</div>`
                : "";
...
function filterCashFlow() {
    ...
    filteredCashFlow =
        cashFlowData.filter(
            record => {
                const searchable =
                    `${record.id} ${record.description} ${record.category} ${record.reference} ${record.account}`.toLowerCase();
...
async function saveCashIn(event) {
    ...
    if (!account) {
        alert(
            "Please select the account where the money will be added."
        );
        return;
    }
    ...
    if (!category) {
        alert(
            "Please enter a cash in category."
        );
        return;
    }
    ...
    try {
        ...
        console.error(
            "[StockMaster] Cash In Firebase error:",
            error
        );

        alert(
            `Unable to save cash in.\n\n${error.message}`
        );
    }
}
...
document
    ...
    ?.addEventListener(
        ...
        () => {
            console.log(
                "[StockMaster] Add Cash In clicked"
            );
