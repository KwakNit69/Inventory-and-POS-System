    ...
    function getSaleProfit(data) {
        ...
        // A paid reservation may still have status "Pending" because
        // its stock has not been deducted yet. Some reservation records
        // store profit as 0 at that stage. Do not let that stored 0
        // override a profit that can be calculated from the sale items.
        ...
        if (items.length) {
            const calculatedProfit = items.reduce(
                (sum, item) => {
                    const quantity =
                    ...
                    // Only use a saved non-zero item profit.
                    // A saved 0 can come from a Pending reservation.
                    if (
                        Number.isFinite(directItemProfit) &&
                        directItemProfit !== 0
                    ) {
        ...
        // Fallback for sale documents that do not contain an items array.
        const total =
            getSaleTotal(data);
