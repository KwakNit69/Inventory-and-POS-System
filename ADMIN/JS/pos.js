...
async function completeSale() {
    ...
    try {
        ...
        const saleItems = cart.map(item => {
            ...
            return { itemId: item.itemId, type: item.type, name: data?.name || item.name, sku: data?.sku || item.sku, price: Number(data?.price ?? item.price), quantity: item.quantity, total: Number(data?.price ?? item.price) * item.quantity, image: data?.image || item.image || "" };
        });
        batch.set(saleRef, {
            transactionNumber,
            customer,
