...
function getPackageAvailability(pkg) {
    if (!Array.isArray(pkg.items) || !pkg.items.length) return 0;
    const capacities = pkg.items.map(item => {
        const product = products.find(product => product.id === item.productId);
        const requiredQuantity = Math.max(1, Number(item.quantity) || 1);
        if (!product) return 0;
        return Math.floor(Math.max(0, Number(product.stock) || 0) / requiredQuantity);
    });
