...
const discount = () => {
    const value =
        parseInt(String(discountInput()?.value || 0).replace(/\D/g, ""), 10) || 0;
...
function getItemData(item) {
    ...
    if (item.type === "package") {
        return packages.find((pkg) => pkg.id === item.itemId);
    }
    if (item.type === "insurance") {
        return insurances.find((insurance) => insurance.id === item.itemId);
    }
    return null;
}
...
function getPackageAvailability(pkg) {
    ...
    return Math.max(
        ...
        ...pkg.items.map((item) => {
            ...
            return product
                ? Math.floor(product.stock / Math.max(1, Number(item.quantity) || 1))
                : 0;
        })
    );
}
...
async function loadData() {
    const [productSnapshot, packageSnapshot, insuranceSnapshot] =
        await Promise.all([
...
function renderProducts() {
    ...
    if (selectedType === "all" || selectedType === "insurance") {
        items.push(...insurances.filter((item) => item.status === "active"));
    }
...
function renderProductCard(item) {
    ...
    if (item.type === "product") {
        ...
        stockText =
            availability <= 0 ? "Unavailable" : `${availability} available`;
    } else {
        stockText = "Available";
    }
...
function addToCart(item) {
    ...
    const existing = cart.find(
        (cartItem) => cartItem.itemId === item.id && cartItem.type === item.type
    );
    ...
    if (existing) {
        ...
        if (
            item.type === "package" &&
            existing.quantity >= getPackageAvailability(item)
        ) {
...
function renderCart() {
    ...
    if (!cart.length) {
        ...
        cart.forEach((item) => {
            ...
            element.innerHTML = `
        <div class="cart-item-image">${data.image ? `<img src="${esc(data.image)}" alt="${esc(data.name)}">` : "▣"}</div>
...
cartItems.addEventListener("click", (event) => {
    ...
    if (button.dataset.action === "increase") {
        ...
        if (
            item.type === "package" &&
            item.quantity >= getPackageAvailability(data)
        ) {
            alert("There are not enough component products for another package.");
            return;
        }
...
document.getElementById("posTypeTabs").addEventListener("click", (event) => {
    ...
    document
        .querySelectorAll(".pos-type-tab")
        .forEach((item) => item.classList.remove("active"));
    button.classList.add("active");

    selectedType = button.dataset.type;
    if (selectedType !== "product") selectedCategory = "all";
...
function bindDiscount() {
    ...
    input.addEventListener("input", () => {
        ...
        if (Number(value) > subtotal()) value = String(Math.floor(subtotal()));
        input.value = value;
        updateTotals();
        updatePayment();
    });
...
function resetSplit() {
    splitPayments = { Cash: 0, GCash: 0, BDO: 0, BIBO: 0, BPI: 0 };
    ["splitCash", "splitGCash", "splitBDO", "splitBIBO", "splitBPI"].forEach(
        (id) => {
...
function splitTotal() {
    return Object.values(splitPayments).reduce(
...
function updateSplit() {
    ...
    if (paymentError) {
        if (overpayment > 0) {
            paymentError.textContent = `Split payment exceeds the amount due by ${money(overpayment)}.`;
        } else if (remaining > 0) {
            paymentError.textContent = `Split payment is ${money(remaining)} short.`;
        } else {
...
function updatePayment() {
    ...
    if (selectedPaymentMethod === "Cash") {
        ...
        document.getElementById("changeAmount").textContent = money(
...
document.querySelectorAll(".payment-method").forEach((button) => {
    button.addEventListener("click", () => {
        ...
        document.getElementById("splitPaymentFields").style.display =
...
async function completeSale() {
    ...
    if (selectedPaymentMethod === "Cash") {
        ...
        paymentBreakdown = {
            ...
            BPI: Number(splitPayments.BPI || 0)
        };
...
function generateReceipt(
    ...
    if (selectedPaymentMethod === "Split") {
        ...
        document.getElementById("receiptSplit").textContent = `Cash ${money(
            paymentBreakdown?.Cash || 0
        )} | GCash ${money(paymentBreakdown?.GCash || 0)} | BDO ${money(
            paymentBreakdown?.BDO || 0
